...
from price_updater_v2 import PriceUpdaterV2
...
# Set up a basic logger if you don't already have one
...
async def add_position(
    ...
    try:
        ...
        # Fetch current price from securities table
        price_query = "SELECT current_price FROM securities WHERE ticker = :ticker"
        security = await database.fetch_one(price_query, {"ticker": position.ticker.upper()})
        
        # Raise error if no current price is available
        if not security or not security['current_price']:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, 
                detail=f"No current price found for ticker {position.ticker}"
            )
        ...
        query = positions.insert().values(
            ...
            price=current_price,  # Use ONLY current market price
...
@app.post("/positions/{account_id}")
async def add_position(account_id: int, position: PositionCreate, current_user: dict = Depends(get_current_user)):
    try:
        # Check if the account belongs to the user
        account_query = accounts.select().where(
            (accounts.c.id == account_id) & 
        ...
        return {"message": "Position added successfully", "position_id": position_id}
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Server error: {str(e)}")        
        
        

@app.post("/market/update-prices")
async def trigger_price_update(current_user: dict = Depends(get_current_user)):
    """Manually trigger a price update"""
    try:
        await smart_market_update()
        
        return {"message": "Price update completed successfully"}
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, 
...
async def add_security(security: SecurityCreate, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        from price_updater import update_company_metrics_for_ticker
        await update_company_metrics_for_ticker(security.ticker.upper())
...
async def update_specific_security(
    ...
    try:
        ...
        if update_data.update_type == "metrics":
            ...
            # Use directly without reimporting
            updater = PriceUpdaterV2()
            days = update_data.days if hasattr(update_data, 'days') else 30  # Default to 30 days
...
@app.get("/system/events")
async def get_system_events(limit: int = 10, current_user: dict = Depends(get_current_user)):
    """Get recent system events"""
    try:
        query = """
            SELECT 
...
async def get_table_stats(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        # Use the same query approach as the working /users endpoint
        # Get users count
